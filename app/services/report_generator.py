...
def build_report_pdf(output_path: Path, result: dict[str, Any], drawing_type: str, sheet_size: str, original_filename: str) -> Path:
    ...
    line_color = Color(0.82, 0.86, 0.9)
    ...
    page.drawString(frame_margin + 4 * mm, frame_margin + 9.5 * mm, 'TrazoCad | Tecno Logisti-K SA')

    page.setFont('Helvetica', 7.5)
    file_label = (original_filename or 'archivo_sin_nombre')[:58]
    page.drawString(frame_margin + 4 * mm, frame_margin + 4.5 * mm, f'Archivo: {file_label}')

    right_x = page_width - frame_margin - 4 * mm
    page.drawRightString(right_x, frame_margin + 9.5 * mm, f'Hoja: {sheet_size.upper()}')
    page.drawRightString(right_x, frame_margin + 4.5 * mm, f'Tipo: {drawing_type} | Fecha: {datetime.now().strftime("%Y-%m-%d %H:%M")}')
